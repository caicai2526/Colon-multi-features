...
for i = 1:100
    ...
    [predict_label_vail(:,i),decision_values_vail(:,2*i-1:2*i),stdevs_vail(:,2*i-1:2*i)]=predict(LDAmodel,data_vail);
...
%     [predict_label_vail(:,i), accuracy_vail(:,i), decision_values_vail(:,i)] = svmpredict(label_vail, data_vail, LDAmodel);
    [predict_label_test(:,i),decision_values_test(:,2*i-1:2*i),stdevs_test(:,2*i-1:2*i)]=predict(LDAmodel,data_test);
...
%     [predict_label_test(:,i), accuracy_test(:,i), decision_values_test(:,2*i-1:2*i)] = svmpredict(test_data_label, data_test, LDAmodel,'-b 1');
    [predict_label_train(:,i),decision_values_train(:,2*i-1:2*i),stdevs_train(:,2*i-1:2*i)]=predict(LDAmodel,train_feature_select);
...
%     [predict_label_train(:,i), accuracy_train(:,i), decision_values_train(:,2*i-1:2*i)] = svmpredict(train_data_label, train_feature_select, LDAmodel,'-b 1');
    ...
    for z = 1:i
        if AUC_test(1,z)<AUC_test(1,i)
            delete([num2str(z),'_LDAmodel','.mat']);
...
num_bestmodel = str2num(LDAmodelfile(1).name(1:end-13));
[predict_label_test_bestmodel, decision_values_test_bestmodel, stdevs_test_bestmodel] = predict(LDAmodel,data_test);
[predict_label_vail_bestmodel, decision_values_vail_bestmodel, stdevs_vail_bestmodel] = predict(LDAmodel,data_vail);
[predict_label_train_bestmodel, decision_values_train_bestmodel, stdevs_train_bestmodel] = predict(LDAmodel,train_feature_select);
